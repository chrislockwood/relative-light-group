...
class RelativeLightGroup(GroupEntity, LightEntity):
    ...
    async def _apply_brightness_with_base(
        ...
        if base_group is None or base_group <= 0:
            rlg_debug_log(
                "light:_apply_brightness_with_base",
                "early_return",
                {
                    "base_group": base_group,
                    "target_brightness": target_brightness,
                    "on_entity_ids": on_entity_ids,
                    "bases_snapshot": dict(self._base_brightness),
            ...
            for state in on_states:
                ...
                if not state.context or state.context.id not in self._last_command_contexts:
                    ...
                    if brightness is not None:
                        ...
                        external_eids.append(state.entity_id)
            # #region agent log
            if external_eids:
                rlg_debug_log(
                    "light:async_update_group_state",
                    "base_from_external_state",
                    {
                        "external_eids": external_eids,
                        "bases_after": dict(self._base_brightness),
                        "context_tail": list(self._last_command_contexts)[-8:],
                    },
                    "H3",
                )
            # #endregion
