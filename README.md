...
- **Representative brightness strategy** — Choose how the group reports brightness from the lights that are on: average (default), median, maximum, or minimum.
- **Member diagnostics** — Optionally expose compact per-light state, brightness, and remembered state attributes on the group for troubleshooting.
...
| **Restore individual brightness** | If enabled, when you turn the group off and on again from the group entity, each light that turns on is set to the brightness it had before the group was turned off. On/off-only lights turn on as usual. If you turn the group on with an explicit brightness, that value is used for all targets instead. |
| **Maintain relative brightness** | If enabled, the group keeps brightness ratios between lights. Proportions are preserved even when the group reaches minimum or maximum brightness. Individual brightness can only be changed directly on each light, not from the group. |
| **Representative brightness** | How the group brightness is calculated from lights that are on. Options are `average` (default), `median`, `max`, and `min`. |
| **Expose member diagnostics** | If enabled, the group exposes compact per-member state and remembered brightness attributes for troubleshooting. Disabled by default to avoid extra recorder/history noise. |
...
| `brightness_strategy` | Representative brightness | Calculate group brightness from on lights using `average`, `median`, `max`, or `min`. | `select` | `average` | No |
| `member_diagnostics` | Expose member diagnostics | Add compact diagnostic attributes for member state, brightness, and remembered values. | `boolean` | `false` | No |
...
When **Expose member diagnostics** is enabled, the group adds these attributes:

| Attribute | Description |
|-----------|-------------|
| `member_states` | Map of each configured member to `on`, `off`, `unavailable`, `unknown`, `disabled`, or `missing`. |
| `member_brightness` | Map of members with a known brightness value. |
| `remembered_on_members` | Members remembered by the on/off restore logic. |
| `remembered_on_members_consistent` | Whether the remembered on/off state came from a consistent member snapshot. |
| `remembered_brightness` | Per-member brightness values remembered before group turn-off. |
| `remembered_brightness_consistent` | Whether remembered brightness came from a consistent member snapshot. |
| `brightness_strategy` | Current representative brightness strategy. |
| `debounce_active` | Whether the group is currently inside its optimistic debounce window. |
